...
const options: ISourceOptions = {
  ...
  detectRetina: true,
  absorbers: [
    { enable: true, opacity: 0, size: { value: 1, density: 1, limit: { radius: 5, mass: 5 } }, position: { x: 110, y: 45 } },
  ],
  emitters: [
    { autoPlay: true, fill: true, life: { wait: true }, rate: { quantity: 5, delay: 0.5 }, position: { x: 110, y: 45 } },
  ],
};
...
export default function AiButton({ href, label = "New Post" }: { href?: string; label?: string }) {
  ...
  return (
    <button
      className="group relative my-2 rounded-full bg-gradient-to-r from-blue-300/30 via-blue-500/30 via-40% to-purple-500/30 p-1 text-white transition-transform hover:scale-110 active:scale-105"
